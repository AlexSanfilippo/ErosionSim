...
//attempt to bring in a texture with height map values (that compute shader can modify and VS can read)
...
{  
    ...
    //gl_Position = projection * view * model * vec4(aPos.x, texCol.r, aPos.z, 1.0);
    ...
    ourColor = 1.5f*flatColor*clamp((1.0f-1.0f*aPos.y),0.33,.8)*(0.5f+aNorm.x)* grassWeight + 1.5f*(clamp(0.25f+aNorm.x,.025f,0.99f)*(1.0f-grassWeight)*vertColor); //*tilt*2.25
    
    //outer condition colors water blue
    if(texCol.g > 0.0001){
        //ourColor = mix(ourColor, vec3(0.f, 0.2f, max(texCol.g,0.45)), 0.25+texCol.g);
        ourColor = mix(0.1f*ourColor, vec3(0.f, 0.2f, 0.45), 0.25+texCol.g);
    }
